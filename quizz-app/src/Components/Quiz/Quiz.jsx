...
let attempts = 5;  // Kullanıcının tahmin hakkı

const getUserGuess = (attempts) => {
  let guess = parseInt(prompt(`Bir sayı tahmin edin (1-100 arası). Kalan hakkınız: ${attempts}`));
  
  if (isNaN(guess) || guess < 1 || guess > 100) {
    alert("Geçersiz giriş! Lütfen 1 ile 100 arasında bir sayı giriniz.");
    return getUserGuess(attempts); // Geçersiz girişte tekrar tahmin iste
  }
  return guess;
};

const checkGuess = (guess, randomNumber) => {
...
while (attempts > 0) {
  let guess = getUserGuess(attempts);
  
  if (checkGuess(guess, randomNumber)) break; // Doğru tahmin edilirse döngüyü kır
  
  attempts--; // Tahmin hakkını azalt

  if (attempts === 0) {
    alert(`Maalesef, hakkınız bitti. Doğru sayı: ${randomNumber}`);
  }
}
...
Açıklama:
Oyun, 1 ile 100 arasında rastgele bir sayı seçer.
Kullanıcıya 5 tahmin hakkı verilir.
Kullanıcı doğru tahminde bulunursa "Tebrikler, doğru tahmin ettiniz!" mesajı görüntülenir ve oyun biter.
Yanlış tahminde ise daha büyük ya da daha küçük sayı girmesi gerektiği söylenir.
Tahmin hakkı biterse, "Üzgünüm, hakkınız bitti" mesajı ve doğru sayı gösterilir.
